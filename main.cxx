...
void runPagerank(const G& x, const H& xt, int repeat) {
  ...
  for (int MC=1, i=0; MC<=5e+7; MC*=i&1? 2:5, i++) {
    auto a2 = pagerankLevelwiseSeq(x, xt, init, {repeat, L1, MC});
    auto e2 = l1Norm(a2.ranks, a1.ranks);
    printf("[%09.3f ms; %03d iters.] [%.4e err.] pagerankLevelwiseSeqL1Norm [min-compute=%d]\n", a2.time, a2.iterations, e2, MC);
  }
  ...
  for (int MC=1, i=0; MC<=5e+7; MC*=i&1? 2:5, i++) {
    auto a4 = pagerankLevelwiseSeq(x, xt, init, {repeat, L2, MC});
    auto e4 = l1Norm(a4.ranks, a1.ranks);
    printf("[%09.3f ms; %03d iters.] [%.4e err.] pagerankLevelwiseSeqL2Norm [min-compute=%d]\n", a4.time, a4.iterations, e4, MC);
  }
  ...
  for (int MC=1, i=0; MC<=5e+7; MC*=i&1? 2:5, i++) {
    auto a6 = pagerankLevelwiseSeq(x, xt, init, {repeat, Li, MC});
    auto e6 = l1Norm(a6.ranks, a1.ranks);
    printf("[%09.3f ms; %03d iters.] [%.4e err.] pagerankLevelwiseSeqLiNorm [min-compute=%d]\n", a6.time, a6.iterations, e6, MC);
  }
}
